...
// Admin-facing — returns every evaluation (Draft and Submitted alike).
// Whether a Draft should actually be shown to the current viewer is a
// permission decision (super admin only), not something this shared hook
// can know, so callers filter by role themselves.
export const useParticipantEvaluations = (participantId) => {
  ...
  const sorted = useMemo(
    () => [...data].sort((a, b) => (a.created_at?.seconds || 0) - (b.created_at?.seconds || 0)),
    [data]
  );
